...
#### 1) Conventional Zero-Shot Learning:

- 여기서는 먼저 CZSL 설정의 최첨단 방법과 방법을 비교합니다. 표 II에 표시된 바와 같이, 우리의 GNDAN은 CUB와 SUN에서 상위 1 정확도에서 다른 비생성 방법을 각각 최소 9.3%와 1.8%만큼 크게 능가합니다. 
  - 이는 GNDAN이 다양한 미세 뿌리 클래스를 구별하기 위한 로컬 차별 기능 표현을 효과적으로 학습할 수 있음을 보여줍니다. 
  - 이러한 결과는 그래프로 탐색된 이중 주의 네트워크를 기반으로 하는 GNDAN의 협력적인 로컬 및 명시적 글로벌 임베딩 학습의 혜택을 받습니다. 거친 입자의 데이터 세트(예: AWA2)의 경우, GNDAN은 여전히 71.0%의 상위 1 정확도로 비슷한 성능을 달성합니다.
  - 종단 간 및 생성 방법과 비교할 때, GNDAN은 CUB과 SUN에서 각각 75.1%와 65.3%의 정확도로 여전히 최고의 결과를 얻을 수 있습니다. 
- 이는 ZSL 작업에서 GNDAN의 장점을 더 보여줍니다.
